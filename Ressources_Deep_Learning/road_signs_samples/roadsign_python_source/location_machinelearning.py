...
sys.path.insert(1, "PATH TO TF OBJECT DETECTION FOLDER")

from utils import label_map_util
from utils import visualization_utils as vis_util

# path to configuration model file
DEFAULT_PATH_TO_MODEL = "ssd_mobilenet_v1_gtsdb3.config"
# Path to frozen detection graph. This is the actual model that is used for the object detection.
DEFAULT_PATH_TO_CKPT = "frozen_inference_graph.pb"

# List of the strings that is used to add correct label for each box.
DEFAULT_PATH_TO_LABELS = "gtsdb3_label_map.pbtxt"
...
class LocationModel (object):
    ...
    def load_model(self, model_path=DEFAULT_PATH_TO_MODEL, ckpt_path=DEFAULT_PATH_TO_CKPT, label_path=DEFAULT_PATH_TO_LABELS, num_classes=DEFAULT_NUM_CLASSES):
        ...
        self.categories = label_map_util.convert_label_map_to_categories(self.label_map, self.max_num_classes=num_classes, use_display_name=True)
        ...
        with self.detection_graph.as_default():
            with tf.Session(graph=self.detection_graph) as sess:
                ...
                (self.boxes, self.scores, self.classes, self.num_detections) = sess.run(
                    ...
                    self.feed_dict={image_tensor: self.image_np_expanded})
