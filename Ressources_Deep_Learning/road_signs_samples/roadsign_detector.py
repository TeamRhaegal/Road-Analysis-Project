...
def roadsign_detector():
    ...
    location_model = location.LocationModel()
    ...
    while(STOP_THREAD != 1):
        try:
            ...
            location_boxes, location_score, location_classes, location_num_detections, location_feed_dict, location_output_image = location_model.detect_roadsigns_from_numpy_array(img)
            ...
            for i in range(location_boxes.shape[1]):
                if (location_score[0][i] > 0.6):
                    ...
                    x1 = int(location_boxes[0][i][1]*location_output_image.shape[1])
                    x2 = int(location_boxes[0][i][3]*location_output_image.shape[1])
                    y1 = int(location_boxes[0][i][0]*location_output_image.shape[0])
                    y2 = int(location_boxes[0][i][2]*location_output_image.shape[0])
                    ...
                    if (DEBUG):
                        ...
                        cv2.rectangle(location_output_image, (x1,y1), (x2,y2), (255,0,0), 1)
            
            if (DEBUG):
                io.imsave("location_result.ppm", location_output_image)
                img = Image.open("location_result.ppm")
                img.show()
